...
- Turnstile 已啟用，`TURNSTILE_ENFORCED = "true"`；CSP 已放行 Cloudflare challenge。
...
- 商品、台幣約定金額、付款方式備註、出貨期限、驗收期限、交易方式。
- 物流 MVP：店到店物流選項、運費誰付、預估運費、長寬高重量、物流提醒、賣方填物流單號。
- 面交安全與未來信任徽章定位已寫入 landing page。
- 取貨資訊 MVP：買方可填姓名、電話、門市；買方查詢遮蔽 PII，賣方 token 可查看寄件所需資料。
...
- 不要在一般使用者文案主打 USDT / USDC、區塊鏈或投資；付款方式只作雙方約定備註。
...
目前不做付款、代收、託管、穩定幣轉帳、AI 仲裁。先做交易條件與證據流程。
